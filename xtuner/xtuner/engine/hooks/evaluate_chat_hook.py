...
class EvaluateChatHook(Hook):
    ...
    def _eval_language(self,
        ...
        for sample_input in self.evaluation_inputs:
            inputs = (self.system + self.instruction).format(
                input=sample_input, round=1, **runner.cfg)
            # 使用 tokenizer() 方法同时获取 input_ids 和 attention_mask
            # 注意：不进行 padding，因为这是单样本生成
            encoded = self.tokenizer(
                inputs,
                return_tensors='pt',
                padding=False,  # 单样本不需要 padding
                truncation=True,
            ...
            if attention_mask is not None:
                ...
                # 如果没有 attention_mask，创建一个全1的mask
                attention_mask = torch.ones_like(input_ids).to(device)
            
            # 准备生成参数
            generate_kwargs = {
                'input_ids': input_ids,
                'attention_mask': attention_mask,  # 明确传递 attention_mask
                'max_new_tokens': max_new_tokens,
                'generation_config': self.gen_config,
                'stopping_criteria': self.stop_criteria,
            ...
            generation_output = self.tokenizer.decode(generated_ids, skip_special_tokens=False)
            runner.logger.info(f'Sample output:\n{generation_output}\n')
            if save_eval_output:
